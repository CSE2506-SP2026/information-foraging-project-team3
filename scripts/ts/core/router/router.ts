...
export class Router {
  ...
  public static HASH_TAG_RESPONDER = (event: Event) => {
    const target = event.target as HTMLAnchorElement;
    const rawHref = target.getAttribute("href") || "";
    if (rawHref.startsWith("#")) {
      event.preventDefault();
      const el = document.getElementById(rawHref.slice(1));
      if (el) {
    ...
    Router.STANDARD_LINK_LISTENER(event as MouseEvent);
  };
  public static EMPTY_RESPONDER = (event: Event) => event.preventDefault();
  public static AT_SYMBOL_RESPONDER = (event: Event) => event.preventDefault();
  ...
  public static STANDARD_LINK_LISTENER(e: MouseEvent) {
    return error(async () => {
      ...
      await HTMLLoader.loadURL(url, Elements.htmlLoc);
  ...
  public static async loadWithPathPrefix(page: string): Promise<boolean> {
    const ret = HTMLLoader.loadURL(Router.pathPrefix + page, Elements.htmlLoc);
    window.dispatchEvent(new CustomEvent("newPageLoad"));
    return ret;
  }
